...
class GameMap extends BallGameObject{
	...
	constructor(root){
		this.root=root;
		this.$playground=$(`
			<div class="ball-game-playground">
	...
	show(){	//open playground
		...
		for(let i=0;i<5;i++){
			this.players.push(
				new Player(this,this.width/2,this.height/2,this.height*0.05,this.get_random_color(),this.height*0.15,false))
		}
	}
	...
	constructor(root){
		this.root=root;
		this.platform="web";
		if(this.root.BallOS)this.platform="app";
		this.username="";
		this.photo="";
...
<div class="ball-game-settings">
	<div class="ball-game-settings-login">
		<div class="ball-game-settings-title">
			登录
		</div>
		...
		<div class="ball-game-settings-error-message">
...
`)
	...
	login_on_remote(){
		...
		$.ajax({
			url:"https://ball.abiding.cn/settings/login/",
			type:"get",
			data:{
				username:username,
				password:password
	...
	register_on_remote(){
		...
		this.$register_password_confirm.empty();

		$.ajax({
			url:"https://ball.abiding.cn/settings/register",
			type:"get",
			data:{
				username:username,
				password:password,
	...
	logout_on_remote(){
		$.ajax({
			url:"https://ball.abiding.cn/settings/logout/",
			type:"get",
			success:function(resp){
				console.log(resp);
				if(resp.result==="success"){
	...
	getinfo(){
		let outer=this;
		$.ajax({
			url:"https://ball.abiding.cn/settings/getinfo/",
			type:"get",
			data:{
